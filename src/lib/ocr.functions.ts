        ...
        messages: [
          {
            role: "system",
            content:
              'Você é um professor particular extremamente cuidadoso. Varra a página inteira de cima para baixo e da esquerda para a direita antes de responder. Responda APENAS com json válido neste formato: {"titulo": string, "questoes": [{"numero": string, "pergunta": string, "resposta": string, "resumo": string, "explicacao": string}]}. Crie exatamente uma entrada para cada questão ou subitem da página, na mesma ordem, sem juntar itens diferentes. "numero" é só o número/letra da questão (ex: "1", "2a"). "pergunta" é o enunciado transcrito. "resposta" é a resposta completa. Em questão de alternativas, escolha somente UMA opção e comece obrigatoriamente a resposta com a identificação inequívoca da opção, no formato "Alternativa A — texto da alternativa" (ou "Option A — ..." se o enunciado estiver em inglês). Nunca liste duas opções como corretas. "resumo" é a mesma resposta em no máximo 8 palavras; se a resposta já for curta e não der para resumir, repita a resposta em "resumo". "explicacao" tem no máximo 2 frases simples. IDIOMA: escreva pergunta, resposta, resumo e explicacao no MESMO idioma do enunciado da questão — se o enunciado estiver em inglês, responda em inglês; se estiver em português, responda em português; para outros idiomas, use o idioma do enunciado. MATEMÁTICA: em questões de matemática, RESOLVA a conta e mostre o desenvolvimento na "resposta": escreva a operação armada passo a passo (ex: "35 x 12 = 35 x 10 + 35 x 2 = 350 + 70 = 420"), incluindo o resultado final; na "explicacao" cite a regra ou propriedade usada. Faça uma conferência final para não omitir, duplicar ou misturar questões. Não invente questões; se algo estiver ilegível, escreva isso na resposta.',
          },
          {
            role: "user",
            content: [
...
export const generateSolvedPhoto = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const texto =
      ...
      handwritingProfile +
      ". Use essa descrição junto com a referência visual. Gere novamente a PRIMEIRA página, idêntica no papel, texto impresso, iluminação, perspectiva, enquadramento e BRILHO/CONTRASTE originais — NÃO altere o brilho, contraste, exposição ou tons da página. Acrescente somente as respostas manuscritas. Escreva a LÁPIS grafite com traço FRACO, CLARO, ACINZENTADO, pressão instável e falhas naturais. A escrita deve ser CURSIVA, porém EXTREMAMENTE FEIA, GARRANCHADA, TORTA, QUEBRADA E DESLEIXADA — o oposto absoluto de letra redonda, bonita, fluida ou caligráfica. INSTRUÇÕES VISUAIS OBRIGATÓRIAS E MÁXIMAS: letras ligadas de forma forçada e desajeitada, com laços TORTOS, PONTUDOS, IRREGULARES e FECHANDO MAL; hastes que saem tortas, entram e saem de outros traços no lugar errado, ligações quebradas ou exageradas. Evite curvas suaves, arcos perfeitos ou arredondamento a TODO CUSTO. Letras pareçam DESMONTADAS/QUEBRADAS: partes soltas, traços desconectados, pontas tortas, laços que não fecham direito, hastes que param no meio e reiniciam. Linha de base MUITO ondulada, subindo e descendo visivelmente; letras claramente desalinhadas, algumas gigantes, outras miúdas, dentro da mesma palavra. Inclinação variando a cada letra. Espaçamento caótico: letras quase coladas, depois buracos grandes. Tremores, retraços, traços extras, rabiscos leves, marcas de correção/apagado e manchas de grafite. Acentos e pontos tortos, traços do 't' e 'f' tortos, círculos de 'a', 'o', 'd' achatados, quadrados ou quebrados. Números desengonçados, com curvas fechadas demais ou abertas demais. TAMANHO: a letra manuscrita da resposta deve ser VISIVELMENTE UM POUCO MAIOR que o título/enunciado impresso da questão, como um aluno que escreve grande. Quanto mais garranchado, feio, quebrado, desleixado, desproporcional e cursivamente desastroso, melhor. NÃO mude a grafia, ortografia ou idioma das palavras: se a resposta estiver em inglês, escreva EM INGLÊS; se estiver em português, escreva EM PORTUGUÊS. NUNCA traduza. As letras devem permanecer reconhecíveis como as mesmas palavras da resposta, apenas visualmente desastrosas. As pequenas marcas X também devem parecer feitas a lápis, fracas e imperfeitas. Não copie o conteúdo da segunda imagem; copie exclusivamente a caligrafia. Não adicione marcas d'água nem texto extra." +
      regras +
      listaRespostas;
